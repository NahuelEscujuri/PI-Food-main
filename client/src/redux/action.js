...
export const getAllRecipes = (only,title)=>{
    return function (dispatch){
        ...
        `https://pi-food-main-production-14c2.up.railway.app/recipes?typeData=${only}`
        return fetch(url)
...
export const getRecipeDetails = (id)=>{
    return function(dispatch){
        fetch(`https://pi-food-main-production-14c2.up.railway.app/recipes/${id}`)
        .then((response) => response.json())
        .then(dataD=>{
...
export function createRecipe(values){
    ...
    return function(dispatch){
        return axios.post(`https://pi-food-main-production-14c2.up.railway.app/recipes`,values)
          .then((data) => { 
...
export const getAllDiets = ()=>{
    return function (dispatch){
        return fetch(`https://pi-food-main-production-14c2.up.railway.app/diets`)
        .then((response) => response.json())
        .then(data=>{
            console.log(data);
            dispatch({
